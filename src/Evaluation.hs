...
solveBinary (defaultValue, terminatingValue, operator) expressions = solveBinaryHelper First leftValues
  where
    leftValues = sort $ flatten operator $ filter (/= defaultValue) $ map simplify expressions
...
solveXor a = decideOnSingles $ sort $ flatten Xor filtered
  where
    simple :: [Expression]
...
flatten :: NAryOperatorType -> [Expression] -> [Expression]
flatten operator [] = []
flatten operator (first:rest) = flattenHelper first ++ flatten operator rest
  where
    flattenHelper value = case value of
      (NAryOperator operator exprs) -> exprs
      otherwise -> [value]